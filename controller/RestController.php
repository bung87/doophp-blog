...
class RestController extends DooController{
	...
	public function page(){
		...
		$ids=$this->db()->fetchAll("SELECT tr.object_id,t.slug,t.name FROM wp_term_relationships tr,wp_term_taxonomy tt,wp_terms t WHERE tt.term_taxonomy_id=tr.term_taxonomy_id AND tr.term_taxonomy_id=t.term_id AND tt.taxonomy='category' ORDER BY tr.object_id DESC LIMIT $start,$limit",null,PDO::FETCH_OBJ);
		$pids=array();
		$cateslug=array();
		$catename=array();
		...
		$num=$pp->count($opts);
		$pager=new DooPager(Doo::conf()->APP_URL.'page',$num,$limit,100);
	...
	public function category(){
		...
		if(isset($this->params['pindex'])){
			$start=($this->params['pindex']-1)*$per;
				...
				$start=0;}
		$ids=$this->db()->fetchAll("SELECT tr.object_id,t.slug,t.name FROM wp_terms t LEFT JOIN wp_term_taxonomy tt ON tt.term_id=t.term_id LEFT JOIN wp_term_relationships tr ON tt.term_taxonomy_id=tr.term_taxonomy_id WHERE t.slug='$category' LIMIT $start,$per",null,PDO::FETCH_OBJ);
	...
	public function comment(){
		...
		if ($ishidden)  echo 'access denied!';
		Doo::loadModel('Comments');
		$comment=new comments;
		$postid=$_POST['comment_post_ID'];
		$comment->comment_post_ID=$postid;
		$comment->comment_author=isset($_POST['author']) ? $_POST['author'] :'';
		$comment->comment_author_email=isset($_POST['email']) ?$_POST['email'] :'';
		$author_url='';

		if(isset($_POST['url'])){
			$author_url=strpos($_POST['url'],'http://') !==false ? $_POST['url'] :'http://'.$_POST['url'];
		}
		$comment->comment_author_url=$author_url;
		...
		$comment->comment_parent=$_POST['comment_parent'];
		if(isset($_COOKIE['userinfo'])){
			$userinfo=json_decode($_COOKIE['userinfo'],true);
			$comment->comment_author=$userinfo['screenName'];
			$comment->comment_author_email=$userinfo['email'];
			$comment->comment_author_url=$userinfo['homepage'];

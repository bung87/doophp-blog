  ...
  wysihtml5.dom.Sandbox = Base.extend(
    ...
    _onLoadIframe: function(iframe) {
	  ...
	  if (scripts) {
		...
		var headNode=iframeDocument.getElementsByTagName('head')[0];
        for (var i=0; i<length; i++) {
			var d=iframeDocument.createElement('script');
			d.src=scripts[i];
			d.type="text/javascript";
			headNode.appendChild(d);
        }
      }
    ...
    _getHtml: function(templateVars) {
      ...
      if (stylesheets) {
        ...
        for (var i=0; i<length; i++) {
          css += '<link rel="stylesheet" href="' + stylesheets[i] + '">';
        }
      }
	   
      templateVars.stylesheets = css;
...
(function(wysihtml5) {
  ...
  wysihtml5.toolbar.Toolbar = Base.extend(
	...
	/*extend for syntax
	   dom.delegate(container, "[data-wysihtml5-syntax]", "click", function(event) {
        var link          = this,
            command       = link.getAttribute("data-wysihtml5-syntax")
          // commandValue  = link.getAttribute("data-wysihtml5-syntax-value");
			sel=rangy.getIframeSelection(jQuery('.wysihtml5-sandbox')[0]);
			ran=rangy.createRange(jQuery('.wysihtml5-sandbox')[0].contentDocument);
			d=document.createElement('pre');
			d.className='brush: '+command;
			ran.setStart(sel.anchorNode,0);
			ran.setEnd(sel.anchorNode,sel.anchorOffset);
			ran.surroundContents(d);
			sel.collapse(d, d.anchorOffset);
			//sel.removeAllRanges();
